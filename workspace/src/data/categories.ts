...
// This is now a fallback for initial load or if firestore fails
...
export const getCategories = async (): Promise<Category[]> => {
  ...
  try {
    ...
    const categoriesWithIcons = categoryList.map(cat => {
        const localCat = localCategories.find(lc => lc.id === cat.id);
        return { ...cat, icon: localCat?.icon };
    });

    cachedCategories = categoriesWithIcons;
    return categoriesWithIcons;
  } catch (error) {
    console.error("Error fetching categories from Firestore, using local fallback:", error);
    return localCategories;
  }
};

export const getCategoryByName = (name: string): Category | undefined => {
  return (cachedCategories || localCategories).find(cat => cat.name.toLowerCase() === name.toLowerCase());
};

export const getCategoryById = (id: string): Category | undefined => {
  return (cachedCategories || localCategories).find(cat => cat.id === id);
};
